...
namespace CoolPay\Catalog\Recurring;

/**
 * Trait Model
 *
 * @package CoolPay\Catalog\Recurring
 */
trait Model {
	...
	public function nextRecurringPayments() {
		...
		$recurring_sql = "SELECT * FROM `" . DB_PREFIX . "order_recurring` `or` INNER JOIN `" . DB_PREFIX . "CoolPay_transaction` qpt ON (qpt.transaction_id = `or`.reference) WHERE `or`.status='" . self::RECURRING_ACTIVE . "'";
